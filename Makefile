...
target pngtarget pdftarget vtarget acrtarget: all.plot.Rout

##################################################################

overleaf:
	git clone https://git.overleaf.com/6569212cbmmjw $@

%.rmk:
	/bin/rm -f $*
	$(MAKE) $*

error: fit.hyb.1.p.nb.1.200.jags.Rout
...
sim.dis.1.bb.p.1.4000.nim.Rout: simulate.CBB.R
...
collect_results_jags.Rout: parameters.CBB.Rout collect_results_jags.R
	$(run-R)

collect_results_nim.Rout: parameters.CBB.Rout collect_results_nim.R
	$(run-R)

collect_results_nimh.Rout: parameters.CBB.Rout collect_results_nimh.R
	$(run-R)
